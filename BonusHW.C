...
void list(char** argv) {
	...
	if (getcwd(cdir, sizeof(cdir)) != NULL) {
		printf("Default Dir", cdir);
	}
    while ((dirent = readdir(cdir)) != NULL)
    	printf("%s\n", dirent->d_name);
	}
...
void help(char **argv){
    printf("");
    printf("Enter showjobs to see all process that are either currently waiting or running (");
    printf("Enter submit to Create a new process to execute the program");
    printf("Enter submithistory to List all the processes that were executed by your job scheduler,");
    printf("Enter List to List ist all the files in the current directory");
    printf("Enter cd to change current directory to the specified directory ");
